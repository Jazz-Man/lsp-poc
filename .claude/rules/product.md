...
lsp-poc is a personal, experimental Language Server Protocol server for the Zed editor — a proof of concept. The owner's framing when it started as a JSON LSP: «Цей проект це мій тестовий JSON LSP для Zed IDE» ("this project is my test JSON LSP for Zed IDE"). The tree has since pivoted to Markdown — the server claims Markdown documents through `tree-sitter-md`, and the extension registers for them. Read every file and every decision through that lens.
...
Not production software. Hardcoded local paths and launching the debug binary are accepted traits of a POC here — leave them alone unless the owner asks for hardening.

Not a replacement for real Markdown tooling. The extension registers the server for Markdown and `.zed/settings.json` selects `zed-lsp-poc` for Markdown files (with `..` keeping Zed's remaining defaults); it is not trying to win users from any real server.

Not a PHP project anymore, despite PHP-era leftovers still in the tree (the structure rule catalogs them). Target all new work at Markdown and `tree-sitter-md`.
