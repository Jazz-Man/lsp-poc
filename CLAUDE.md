...
- wasm `cdylib` on `zed_extension_api`. `language_server_command()` in `src/lib.rs` launches `<worktree-root>/target/debug/lsp-poc` with no arguments — the profile is hardcoded (no settings switch), so only a debug build needs to exist.
